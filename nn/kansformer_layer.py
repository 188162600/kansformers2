...
from .kan_quant_noise import kan_quant_noise
from .kan_layer import KANLayer
# from ..model.transformer_config import TransformerConfig

class KansformerEncoderLayerBase(nn.Module):
    """Encoder layer block.
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        return kan_quant_noise(
            KANLayer(input_dim, output_dim), p=q_noise, block_size=qn_block_size
        )
    ...
    def _prune_fc_layer(self, remove_index: List[int]):
        ...
        self.fc1 = kan_quant_noise(
            KANLayer(self.fc1.in_features, self.fc1.out_features - len(remove_index)),
            p=self.quant_noise,
        ...
        self.fc2 = kan_quant_noise(
            KANLayer(self.fc2.in_features - len(remove_index), self.fc2.out_features),
            p=self.quant_noise,
            block_size=self.quant_noise_block_size,
        )
    ...
    def forward(self,x,*args,**kwargs):
        return self.final_layer_norm( self.fc2(self.fc1(x)))


# backward compatible with the legacy argparse format
class KansformerEncoderLayer(KansformerEncoderLayerBase):
    def __init__(self, args):
        super().__init__(KansformerConfig.from_namespace(args))
        self.args = args

    def build_self_attention(self, embed_dim, args):
        return super().build_self_attention(
            embed_dim, KansformerConfig.from_namespace(args)
        )


class KansformerDecoderLayerBase(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        return kan_quant_noise(KANLayer(input_dim, output_dim), q_noise, qn_block_size)
    ...
    def forward(
        ...
        return x, attn, None
    def forward(self,x,*args,**kwargs):
        return self.fc2(self.ffn_layernorm( self.fc1(x))),None,None
    def make_generation_fast_(self, need_attn: bool = False, **kwargs):
        self.need_attn = need_attn


# backward compatible with the legacy argparse format
class KansformerDecoderLayer(KansformerDecoderLayerBase):
    def __init__(
        self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False
    ):
        super().__init__(
            KansformerConfig.from_namespace(args),
